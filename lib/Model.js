...
class Model {
    ...
    // returns a Redis-like (array) representation of the data,
    // excluding default values and empty fields
    // useful for 'hmset' calls, etc.
    toArray() {
    ...
    // white-list model fields to expose
    select(fields, includePrivates) {
        if (typeof fields === 'string')
            fields = fields.trim().split(/[\s,]+/g)

        assert(Array.isArray(fields), 'model.select() requires an array of fields to select')
        ...
        for (let i = fields.length; i--;) {
            const key = fields[ i ],
                  val = changes[ key ] || data[ key ] || defaults[ key ]

            if (val !== undefined && (includePrivates || !(key in internals)))
                result[ key ] = val
        }
